...
class Server(Node):
    ...
    def timer_callback(self):
        ...
        if 'B' in buttons and buttons['B'][0] > 0.0:
            self.get_logger().info('B pressed.')
            try:
                msg = Float32MultiArray()
                matrix_ri = self.oculus_reader.last_transforms.get("r")
                if self.oculus_reader.running and matrix_ri is not None:
                    x = matrix_ri[0][3]
                    y = matrix_ri[1][3]
                    z = matrix_ri[2][3]
                    matrix_rot = matrix_ri[:3,:3]
                    pitch = math.atan2(-matrix_rot[2][0], math.sqrt(matrix_rot[0][0]*matrix_rot[0][0] + matrix_rot[1][0]*matrix_rot[1][0]))
                    yaw = math.atan2(matrix_rot[1][0], matrix_rot[0][0])
                    roll = math.atan2(matrix_rot[2][1], matrix_rot[2][2])
                    # print("x:",x,"y:",y,"z:",z,"pitch:",pitch,"yaw:",yaw,"roll:",roll)
                    msg.data = [x, y, z, pitch, yaw, roll]
